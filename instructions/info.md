...
## Filesize Apps
1. Coreutils du: used to define (colorized!) function fs() in ~/.functions
2. [gdu](https://github.com/dundee/gdu):
   1. Full fledged browser
   2. Binary name conflicts with coreutils du, so is accesible via symlink /opt/homebrew/bin/gfs
      ```bash
      ln -s ${HOMEBREW_PREFIX}/Cellar/gdu/5.13.0/bin/gdu /opt/homebrew/bin/gfs
      ```
